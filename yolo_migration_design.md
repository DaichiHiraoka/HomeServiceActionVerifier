...
現在の `grip_detector` プロトタイプでは、暫定的に `lift` の前段として「把持中移動」を判定する。これは、手の把持姿勢スコア、手と物体bboxの接触スコア、追跡物体の速度または初期位置からの変位を統合したものである。手が物体に重なっているだけの状態は `contact` または候補に留め、物体が動いた場合にのみ確定スコアを上げる。
